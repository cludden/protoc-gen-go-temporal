...
func (a *exampleActivities) GetFooProgress(ctx context.Context, input *v11.QueryRequest) (*v1.GetFooProgressResponse, error) {
	return a.client.GetFooProgress(ctx, input.GetWorkflowId(), input.GetRunId())
}

// SetFooProgress executes a(n) example.v1.Example.SetFooProgress signal via an activity
func (a *exampleActivities) SetFooProgress(ctx context.Context, input *v11.SignalRequest) error {
	// unmarshal signal request
	var req v1.SetFooProgressRequest
	if err := input.Request.UnmarshalTo(&req); err != nil {
		return err
	}
	return a.client.SetFooProgress(ctx, input.GetWorkflowId(), input.GetRunId(), &req)
}
