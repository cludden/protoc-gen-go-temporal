...
// ExampleClientProviderInput describes a(n) example.helloworld.v1.Example xns activity invocation and is provided to a ExampleClientProvider so that a
...
type ExampleClientProviderInput struct {
	...
	// e.g. "example.helloworld.v1.Example.CancelWorkflow"
	ActivityName string
...
// RegisterExampleActivities registers example.helloworld.v1.Example cross-namespace activities using a static client. When a
...
// RegisterExampleActivitiesWithClientProvider registers example.helloworld.v1.Example cross-namespace activities, using the given ExampleClientProvider to
...
func (r *helloRun) Get(ctx workflow.Context) (*v1.HelloResponse, error) {
	...
	if r.future == nil {
		rr := GetHelloAsync(ctx, r.id, "", NewGetHelloOptions().WithParentClosePolicy(r.parentClosePolicy).WithHeartbeatInterval(r.heartbeatInterval)).(*helloRun)
		r.future = rr.future
		r.cancel = cancel
	}
...
func GetHello(ctx workflow.Context, workflowID string, runID string, options ...*GetHelloOptions) (out *v1.HelloResponse, err error) {
	out, err = GetHelloAsync(ctx, workflowID, runID, options...).Get(ctx)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func GetHelloAsync(ctx workflow.Context, workflowID string, runID string, options ...*GetHelloOptions) HelloRun {
	...
	var opt *GetHelloOptions
	if len(options) > 0 && options[0] != nil {
		opt = options[0]
	} else {
		opt = NewGetHelloOptions()
	}
...
func (a *exampleActivities) CancelWorkflow(ctx context.Context, workflowID string, runID string) error {
	c, err := a.getClient(ctx, &ExampleClientProviderInput{
		ActivityName: "example.helloworld.v1.Example.CancelWorkflow",
		RunID:        runID,
		WorkflowID:   workflowID,
	})
	if err != nil {
...
func (a *exampleActivities) GetHello(ctx context.Context, input *xnsv1.GetWorkflowRequest) (out *v1.HelloResponse, err error) {
	...
	c, err := a.getClient(ctx, &ExampleClientProviderInput{
		ActivityName: "example.helloworld.v1.Example.GetHello",
	...
	defer cancel()
	run := c.GetHello(actx, input.GetWorkflowId(), input.GetRunId())
...
func (a *exampleActivities) Hello(ctx context.Context, input *xnsv1.WorkflowRequest) (resp *v1.HelloResponse, err error) {
	...
	var run v1.HelloRun
	run, err = c.HelloAsync(actx, &req, v1.NewHelloOptions().WithStartWorkflowOptions(
...
func (a *exampleActivities) Goodbye(ctx context.Context, input *xnsv1.SignalRequest) (err error) {
	...
	c, err := a.getClient(ctx, &ExampleClientProviderInput{
		ActivityName: "example.helloworld.v1.Example.Goodbye",
		Requests:     []proto.Message{&req},
		RunID:        input.GetRunId(),
		WorkflowID:   input.GetWorkflowId(),
	})
	...
	doneCh := make(chan struct{})
	go func() {
		err = c.Goodbye(ctx, input.GetWorkflowId(), input.GetRunId(), &req)
		close(doneCh)
	}()
