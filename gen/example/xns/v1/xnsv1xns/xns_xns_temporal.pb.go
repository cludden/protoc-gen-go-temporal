...
func (opts *XnsOptions) filter(name string) string {
...
func RegisterXnsActivities(r worker.ActivityRegistry, c v1.XnsClient, opts ...XnsOptions) {
	if len(opts) > 0 {
		xnsOptions = &opts[0]
	}
	a := &xnsActivities{c}
	if name := xnsOptions.filter("example.xns.v1.Xns.CancelWorkflow"); name != "" {
		r.RegisterActivityWithOptions(a.CancelWorkflow, activity.RegisterOptions{Name: name})
	}
	if name := xnsOptions.filter(v1.ProvisionFooWorkflowName); name != "" {
...
func ProvisionFooAsync(ctx workflow.Context, req *v1.ProvisionFooRequest, opts ...*ProvisionFooWorkflowOptions) (ProvisionFooRun, error) {
	activityName := xnsOptions.filter(v1.ProvisionFooWorkflowName)
	if activityName == "" {
...
func CancelXnsWorkflowAsync(ctx workflow.Context, workflowID string, runID string) workflow.Future {
	activityName := xnsOptions.filter("example.xns.v1.Xns.CancelWorkflow")
	if activityName == "" {
...
func RegisterExampleActivities(r worker.ActivityRegistry, c v1.ExampleClient, opts ...ExampleOptions) {
	...
	if name := exampleOptions.filter("example.xns.v1.Example.CancelWorkflow"); name != "" {
		r.RegisterActivityWithOptions(a.CancelWorkflow, activity.RegisterOptions{Name: name})
	}
	...
	if name := exampleOptions.filter("example.xns.v1.Example.CreateFooWithSetFooProgress"); name != "" {
...
func CreateFooWithSetFooProgressAsync(ctx workflow.Context, req *v1.CreateFooRequest, signal *v1.SetFooProgressRequest, opts ...*CreateFooWorkflowOptions) (CreateFooRun, error) {
	activityName := exampleOptions.filter("example.xns.v1.Example.CreateFooWithSetFooProgress")
	if activityName == "" {
		return nil, temporal.NewNonRetryableApplicationError(
...
func CancelExampleWorkflowAsync(ctx workflow.Context, workflowID string, runID string) workflow.Future {
	activityName := exampleOptions.filter("example.xns.v1.Example.CancelWorkflow")
	if activityName == "" {
		f, s := workflow.NewFuture(ctx)
		s.SetError(temporal.NewNonRetryableApplicationError(
