...
func GetProvisionFoo(ctx workflow.Context, workflowID string, runID string) (out *v1.ProvisionFooResponse, err error) {
	out, err = GetProvisionFooAsync(ctx, workflowID, runID).Get(ctx)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func GetProvisionFooAsync(ctx workflow.Context, workflowID string, runID string) ProvisionFooRun {
...
func (a *xnsActivities) GetProvisionFoo(ctx context.Context, input *xnsv1.GetWorkflowRequest) (out *v1.ProvisionFooResponse, err error) {
	...
	go func() {
		defer close(done)
		out, err = a.client.GetProvisionFoo(ctx, input.GetWorkflowId(), input.GetRunId()).Get(ctx)
	}()
	for {
		select {
		...
		case <-activity.GetWorkerStopChannel(ctx):
			return nil, xnsOptions.convertError(temporal.NewApplicationError("worker is stopping", "WorkerStopping"))
		case <-ctx.Done():
			return nil, xnsOptions.convertError(err)
		}
	}
...
func GetCreateFoo(ctx workflow.Context, workflowID string, runID string) (out *v1.CreateFooResponse, err error) {
	out, err = GetCreateFooAsync(ctx, workflowID, runID).Get(ctx)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func GetCreateFooAsync(ctx workflow.Context, workflowID string, runID string) CreateFooRun {
...
func (a *exampleActivities) GetCreateFoo(ctx context.Context, input *xnsv1.GetWorkflowRequest) (out *v1.CreateFooResponse, err error) {
	...
	go func() {
		defer close(done)
		out, err = a.client.GetCreateFoo(ctx, input.GetWorkflowId(), input.GetRunId()).Get(ctx)
	}()
	for {
		select {
		case <-time.After(heartbeatInterval):
