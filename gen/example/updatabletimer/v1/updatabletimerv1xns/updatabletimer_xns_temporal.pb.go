...
// ExampleClientProviderInput describes a(n) example.updatabletimer.v1.Example xns activity invocation and is provided to a ExampleClientProvider so that a
...
type ExampleClientProviderInput struct {
	...
	// e.g. "example.updatabletimer.v1.Example.CancelWorkflow"
	ActivityName string
...
// RegisterExampleActivities registers example.updatabletimer.v1.Example cross-namespace activities using a static client. When a
...
// RegisterExampleActivitiesWithClientProvider registers example.updatabletimer.v1.Example cross-namespace activities, using the given ExampleClientProvider to
...
func RegisterExampleActivitiesWithClientProvider(r worker.ActivityRegistry, provider ExampleClientProvider, options ...*ExampleOptions) {
	...
	if name := exampleOptions.filterActivity("example.updatabletimer.v1.Example.GetUpdatableTimer"); name != "" {
		r.RegisterActivityWithOptions(a.GetUpdatableTimer, activity.RegisterOptions{Name: name})
	}
...
func (opts *UpdatableTimerWorkflowOptions) Build(ctx workflow.Context, input *v1.UpdatableTimerInput) (workflow.Context, *xnsv1.WorkflowRequest, error) {
	...
	if swo.ID == "" {
		if err := workflow.SideEffect(ctx, func(ctx workflow.Context) any {
			id, err := expression.EvalExpression(v1.UpdatableTimerIdexpression, input.ProtoReflect())
			if err != nil {
...
func (opts *UpdatableTimerWorkflowOptions) WithHeartbeatTimeout(d time.Duration) *UpdatableTimerWorkflowOptions {
...
func (r *updatableTimerRun) Future() workflow.Future {
	if r.future == nil {
		rr := GetUpdatableTimerAsync(r.ctx, r.id, "").(*updatableTimerRun)
		r.future = rr.future
		r.cancel = rr.cancel
	}
	return r.future
...
func (r *updatableTimerRun) Get(ctx workflow.Context) error {
	...
	if r.future == nil {
		rr := GetUpdatableTimerAsync(ctx, r.id, "", NewGetUpdatableTimerOptions().WithParentClosePolicy(r.parentClosePolicy).WithHeartbeatInterval(r.heartbeatInterval)).(*updatableTimerRun)
		r.future = rr.future
		r.cancel = cancel
	}
...
// UpdatableTimer describes an updatable timer workflow
func UpdatableTimerAsync(ctx workflow.Context, input *v1.UpdatableTimerInput, opts ...*UpdatableTimerWorkflowOptions) (UpdatableTimerRun, error) {
	...
	var opt *UpdatableTimerWorkflowOptions
	if len(opts) > 0 && opts[0] != nil {
		opt = opts[0]
	} else {
		opt = NewUpdatableTimerWorkflowOptions()
	}
...
// GetUpdatableTimer returns a(n) UpdatableTimer workflow execution
func GetUpdatableTimer(ctx workflow.Context, workflowID string, runID string, options ...*GetUpdatableTimerOptions) (err error) {
	err = GetUpdatableTimerAsync(ctx, workflowID, runID, options...).Get(ctx)
	if err != nil {
		return err
	}
	return nil
...
// GetUpdatableTimerAsync returns a handle to a(n) UpdatableTimer workflow execution
func GetUpdatableTimerAsync(ctx workflow.Context, workflowID string, runID string, options ...*GetUpdatableTimerOptions) UpdatableTimerRun {
	activityName := exampleOptions.filterActivity("example.updatabletimer.v1.Example.GetUpdatableTimer")
	if activityName == "" {
	...
	var opt *GetUpdatableTimerOptions
	if len(options) > 0 && options[0] != nil {
		opt = options[0]
	} else {
		opt = NewGetUpdatableTimerOptions()
	}
...
// GetUpdatableTimerOptions are used to configure a(n) UpdatableTimer workflow execution getter activity
type GetUpdatableTimerOptions struct {
	activityOptions   *workflow.ActivityOptions
	heartbeatInterval time.Duration
	parentClosePolicy enumsv1.ParentClosePolicy
}

// NewGetUpdatableTimerOptions initializes a new GetUpdatableTimerOptions value
func NewGetUpdatableTimerOptions() *GetUpdatableTimerOptions {
	return &GetUpdatableTimerOptions{}
}

// Build initializes the activity context and input
func (opt *GetUpdatableTimerOptions) Build(ctx workflow.Context, workflowID string, runID string) (workflow.Context, *xnsv1.GetWorkflowRequest, error) {
...
// WithActivityOptions can be used to customize the activity options
func (o *GetUpdatableTimerOptions) WithActivityOptions(ao workflow.ActivityOptions) *GetUpdatableTimerOptions {
	o.activityOptions = &ao
	return o
}

// WithHeartbeatInterval can be used to customize the activity heartbeat interval
func (o *GetUpdatableTimerOptions) WithHeartbeatInterval(d time.Duration) *GetUpdatableTimerOptions {
	o.heartbeatInterval = d
	return o
}

// WithParentClosePolicy can be used to customize the cancellation propagation behavior
func (o *GetUpdatableTimerOptions) WithParentClosePolicy(policy enumsv1.ParentClosePolicy) *GetUpdatableTimerOptions {
...
func (a *exampleActivities) CancelWorkflow(ctx context.Context, workflowID string, runID string) error {
	c, err := a.getClient(ctx, &ExampleClientProviderInput{
		ActivityName: "example.updatabletimer.v1.Example.CancelWorkflow",
		RunID:        runID,
		WorkflowID:   workflowID,
	})
	if err != nil {
...
// GetUpdatableTimer retrieves a(n) UpdatableTimer workflow via an activity
func (a *exampleActivities) GetUpdatableTimer(ctx context.Context, input *xnsv1.GetWorkflowRequest) (err error) {
	...
	c, err := a.getClient(ctx, &ExampleClientProviderInput{
		ActivityName: "example.updatabletimer.v1.Example.GetUpdatableTimer",
	...
	defer cancel()
	run := c.GetUpdatableTimer(actx, input.GetWorkflowId(), input.GetRunId())
...
func (a *exampleActivities) UpdatableTimer(ctx context.Context, input *xnsv1.WorkflowRequest) (err error) {
	...
	run, err = c.UpdatableTimerAsync(actx, &req, v1.NewUpdatableTimerOptions().WithStartWorkflowOptions(
...
func (a *exampleActivities) GetWakeUpTime(ctx context.Context, input *xnsv1.QueryRequest) (resp *v1.GetWakeUpTimeOutput, err error) {
	c, err := a.getClient(ctx, &ExampleClientProviderInput{
		ActivityName: "example.updatabletimer.v1.Example.GetWakeUpTime",
		RunID:        input.GetRunId(),
		WorkflowID:   input.GetWorkflowId(),
	})
	if err != nil {
	...
	doneCh := make(chan struct{})
	go func() {
		resp, err = c.GetWakeUpTime(ctx, input.GetWorkflowId(), input.GetRunId())
		close(doneCh)
	}()
...
func (a *exampleActivities) UpdateWakeUpTime(ctx context.Context, input *xnsv1.SignalRequest) (err error) {
	...
	c, err := a.getClient(ctx, &ExampleClientProviderInput{
		ActivityName: "example.updatabletimer.v1.Example.UpdateWakeUpTime",
		Requests:     []proto.Message{&req},
		RunID:        input.GetRunId(),
		WorkflowID:   input.GetWorkflowId(),
	})
	...
	doneCh := make(chan struct{})
	go func() {
		err = c.UpdateWakeUpTime(ctx, input.GetWorkflowId(), input.GetRunId(), &req)
		close(doneCh)
	}()
