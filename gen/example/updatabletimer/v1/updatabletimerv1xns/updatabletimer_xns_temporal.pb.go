...
func GetUpdatableTimer(ctx workflow.Context, workflowID string, runID string) (err error) {
	err = GetUpdatableTimerAsync(ctx, workflowID, runID).Get(ctx)
	if err != nil {
		return err
	}
	return nil
...
func GetUpdatableTimerAsync(ctx workflow.Context, workflowID string, runID string) UpdatableTimerRun {
...
func (a *exampleActivities) GetUpdatableTimer(ctx context.Context, input *xnsv1.GetWorkflowRequest) (err error) {
	...
	go func() {
		defer close(done)
		err = a.client.GetUpdatableTimer(ctx, input.GetWorkflowId(), input.GetRunId()).Get(ctx)
	}()
	for {
		select {
