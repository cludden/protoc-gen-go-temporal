...
func GetSchedule(ctx workflow.Context, workflowID string, runID string) (out *v1.ScheduleOutput, err error) {
	out, err = GetScheduleAsync(ctx, workflowID, runID).Get(ctx)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func GetScheduleAsync(ctx workflow.Context, workflowID string, runID string) ScheduleRun {
...
func (a *exampleActivities) GetSchedule(ctx context.Context, input *xnsv1.GetWorkflowRequest) (out *v1.ScheduleOutput, err error) {
	...
	go func() {
		defer close(done)
		out, err = a.client.GetSchedule(ctx, input.GetWorkflowId(), input.GetRunId()).Get(ctx)
	}()
	for {
		select {
