...
func GetSearchAttributes(ctx workflow.Context, workflowID string, runID string) (err error) {
	err = GetSearchAttributesAsync(ctx, workflowID, runID).Get(ctx)
	if err != nil {
		return err
	}
	return nil
...
func GetSearchAttributesAsync(ctx workflow.Context, workflowID string, runID string) SearchAttributesRun {
...
func (a *exampleActivities) GetSearchAttributes(ctx context.Context, input *xnsv1.GetWorkflowRequest) (err error) {
	...
	go func() {
		defer close(done)
		err = a.client.GetSearchAttributes(ctx, input.GetWorkflowId(), input.GetRunId()).Get(ctx)
	}()
	for {
		select {
