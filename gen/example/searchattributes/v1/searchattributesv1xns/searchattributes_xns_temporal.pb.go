...
func RegisterExampleActivities(r worker.ActivityRegistry, c v1.ExampleClient, opts ...ExampleOptions) {
	...
	if name := exampleOptions.filter("example.searchattributes.v1.Example.CancelWorkflow"); name != "" {
		r.RegisterActivityWithOptions(a.CancelWorkflow, activity.RegisterOptions{Name: name})
	}
	if name := exampleOptions.filter(v1.SearchAttributesWorkflowName); name != "" {
...
func SearchAttributesAsync(ctx workflow.Context, req *v1.SearchAttributesInput, opts ...*SearchAttributesWorkflowOptions) (SearchAttributesRun, error) {
	activityName := exampleOptions.filter(v1.SearchAttributesWorkflowName)
	if activityName == "" {
...
func CancelExampleWorkflowAsync(ctx workflow.Context, workflowID string, runID string) workflow.Future {
	activityName := exampleOptions.filter("example.searchattributes.v1.Example.CancelWorkflow")
	if activityName == "" {
