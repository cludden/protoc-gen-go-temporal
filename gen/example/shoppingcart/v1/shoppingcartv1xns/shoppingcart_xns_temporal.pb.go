...
func GetShoppingCart(ctx workflow.Context, workflowID string, runID string) (out *v1.ShoppingCartOutput, err error) {
	out, err = GetShoppingCartAsync(ctx, workflowID, runID).Get(ctx)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func GetShoppingCartAsync(ctx workflow.Context, workflowID string, runID string) ShoppingCartRun {
...
func (a *shoppingCartActivities) GetShoppingCart(ctx context.Context, input *xnsv1.GetWorkflowRequest) (out *v1.ShoppingCartOutput, err error) {
	...
	go func() {
		defer close(done)
		out, err = a.client.GetShoppingCart(ctx, input.GetWorkflowId(), input.GetRunId()).Get(ctx)
	}()
	for {
		select {
...
func (a *shoppingCartActivities) ShoppingCartWithUpdateCart(ctx context.Context, input *xnsv1.UpdateWithStartRequest) (out *v1.UpdateCartOutput, err error) {
	...
	handle, run, err := a.client.ShoppingCartWithUpdateCartAsync(ctx, &req, &update, v1.NewShoppingCartWithUpdateCartOptions().WithShoppingCartOptions(v1.NewShoppingCartOptions().WithStartWorkflowOptions(swo)).WithUpdateCartOptions(v1.NewUpdateCartOptions().WithUpdateWorkflowOptions(uwo)))
	if err != nil {
		return nil, shoppingCartOptions.convertError(err)
	}
