...
// versions:
//
//	protoc-gen-go_temporal 0.5.1-next (3dc2c9f1e8b460fee34977d82c25a95e60acec39)
//	go go1.19.5
//	protoc (unknown)
//
...
type Client interface {
	// SomeWorkflow1 executes a SomeWorkflow1 workflow and blocks until error or response received
	...
	// SomeWorkflow2 executes a SomeWorkflow2 workflow and blocks until error or response received
	...
	// SomeWorkflow3 executes a SomeWorkflow3 workflow and blocks until error or response received
...
func (c *workflowClient) ExecuteSomeWorkflow3(ctx context.Context, opts *client.StartWorkflowOptions, req *SomeWorkflow3Request) (SomeWorkflow3Run, error) {
	...
	if opts.ID == "" {
		opts.ID = fmt.Sprintf("%v/%v", req.GetId(), req.GetRequestVal())
	}
	if !strings.HasPrefix(opts.ID, SomeWorkflow3IDPrefix) {
		opts.ID = fmt.Sprintf("%s/%s", SomeWorkflow3IDPrefix, opts.ID)
	}
...
// SomeWorkflow1Workflow describes a SomeWorkflow1 workflow implementation
...
func SomeWorkflow1Child(ctx workflow.Context, opts *workflow.ChildWorkflowOptions, req *SomeWorkflow1Request) SomeWorkflow1ChildRun {
	...
	return SomeWorkflow1ChildRun{
		Future: workflow.ExecuteChildWorkflow(ctx, "SomeWorkflow1WorkflowName", req),
	}
}
...
// SomeWorkflow2Workflow describes a SomeWorkflow2 workflow implementation
...
func SomeWorkflow2Child(ctx workflow.Context, opts *workflow.ChildWorkflowOptions) SomeWorkflow2ChildRun {
	...
	return SomeWorkflow2ChildRun{
		Future: workflow.ExecuteChildWorkflow(ctx, "SomeWorkflow2WorkflowName", nil),
	}
}
...
// SomeWorkflow3Workflow describes a SomeWorkflow3 workflow implementation
...
func SomeWorkflow3Child(ctx workflow.Context, opts *workflow.ChildWorkflowOptions, req *SomeWorkflow3Request) SomeWorkflow3ChildRun {
	...
	if opts.WorkflowID == "" {
		opts.WorkflowID = fmt.Sprintf("%v/%v", req.GetId(), req.GetRequestVal())
	}
	if !strings.HasPrefix(opts.WorkflowID, SomeWorkflow3IDPrefix) {
		opts.WorkflowID = fmt.Sprintf("%s/%s", SomeWorkflow3IDPrefix, opts.WorkflowID)
	}
	if opts.WorkflowIDReusePolicy == v1.WORKFLOW_ID_REUSE_POLICY_UNSPECIFIED {
		opts.WorkflowIDReusePolicy = v1.WORKFLOW_ID_REUSE_POLICY_ALLOW_DUPLICATE
	}
	...
	return SomeWorkflow3ChildRun{
		Future: workflow.ExecuteChildWorkflow(ctx, "SomeWorkflow3WorkflowName", req),
	}
}
