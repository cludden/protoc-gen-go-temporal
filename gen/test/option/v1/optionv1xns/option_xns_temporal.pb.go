...
func GetWorkflowWithInput(ctx workflow.Context, workflowID string, runID string) (err error) {
	err = GetWorkflowWithInputAsync(ctx, workflowID, runID).Get(ctx)
	if err != nil {
		return err
	}
	return nil
...
func GetWorkflowWithInputAsync(ctx workflow.Context, workflowID string, runID string) WorkflowWithInputRun {
...
func (a *testActivities) GetWorkflowWithInput(ctx context.Context, input *xnsv1.GetWorkflowRequest) (err error) {
	...
	go func() {
		defer close(done)
		err = a.client.GetWorkflowWithInput(ctx, input.GetWorkflowId(), input.GetRunId()).Get(ctx)
	}()
	for {
		select {
		...
		case <-activity.GetWorkerStopChannel(ctx):
			return testOptions.convertError(temporal.NewApplicationError("worker is stopping", "WorkerStopping"))
		case <-ctx.Done():
