...
func buildSleep(ctor func(workflow.Context, *SleepWorkflowInput) (SleepWorkflow, error)) func(workflow.Context, *SleepRequest) error {
	return func(ctx workflow.Context, req *SleepRequest) error {
		...
		ctx = patch.WithDefaultTaskQueue(workflow.WithValue, ctx, ServerTaskQueue, workflow.GetInfo(ctx).TaskQueueName)
...
func (o *SleepChildOptions) Build(ctx workflow.Context, req protoreflect.Message) (workflow.ChildWorkflowOptions, error) {
	...
	if v := o.taskQueue; v != nil {
		...
		if workflow.GetVersion(ctx, "cludden_protoc-gen-go-temporal_77_use-parent-task-queue", workflow.DefaultVersion, 1) == 1 {
			if tq := patch.DefaultTaskQueue(ctx, ServerTaskQueue); tq != "" && tq != workflow.GetInfo(ctx).TaskQueueName {
				opts.TaskQueue = tq
			}
		} else {
			opts.TaskQueue = ServerTaskQueue
...
func buildCallSleep(ctor func(workflow.Context, *CallSleepWorkflowInput) (CallSleepWorkflow, error)) func(workflow.Context, *CallSleepRequest) error {
	return func(ctx workflow.Context, req *CallSleepRequest) error {
		...
		// inject default task queue and override into workflow context
		ctx = patch.WithDefaultTaskQueue(workflow.WithValue, ctx, ClientTaskQueue, workflow.GetInfo(ctx).TaskQueueName)
		wf, err := ctor(ctx, input)
		if err != nil {
			return err
		}
...
func (o *CallSleepChildOptions) Build(ctx workflow.Context, req protoreflect.Message) (workflow.ChildWorkflowOptions, error) {
	...
	if v := o.taskQueue; v != nil {
		...
		// more info: https://cludden.github.io/protoc-gen-go-temporal/docs/guides/patches#pv_77-use-parent-task-queue
		if workflow.GetVersion(ctx, "cludden_protoc-gen-go-temporal_77_use-parent-task-queue", workflow.DefaultVersion, 1) == 1 {
			if tq := patch.DefaultTaskQueue(ctx, ClientTaskQueue); tq != "" && tq != workflow.GetInfo(ctx).TaskQueueName {
				opts.TaskQueue = tq
			}
		} else {
			opts.TaskQueue = ClientTaskQueue
...
func UnmarshalCliFlagsToCallSleepRequest(cmd *v2.Context) (*CallSleepRequest, error) {
	...
	if cmd.IsSet("start-workflow-options") {
		hasValues = true
		var v xnsv1.StartWorkflowOptions
