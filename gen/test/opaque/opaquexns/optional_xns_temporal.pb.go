// Code generated by protoc-gen-go_temporal. DO NOT EDIT.
// versions:
//
//	protoc-gen-go_temporal 1.15.1-next (6549a3aa2e5c335647818f1d937579ecd7a9ca9e)
//	go go1.24.0
//	protoc (unknown)
//
// source: test/opaque/optional.proto
package opaquexns

import (
	"context"
	"errors"
	"fmt"
	temporalv1 "github.com/cludden/protoc-gen-go-temporal/gen/temporal/v1"
	xnsv1 "github.com/cludden/protoc-gen-go-temporal/gen/temporal/xns/v1"
	opaque "github.com/cludden/protoc-gen-go-temporal/gen/test/opaque"
	xns "github.com/cludden/protoc-gen-go-temporal/pkg/xns"
	uuid "github.com/google/uuid"
	enumsv1 "go.temporal.io/api/enums/v1"
	activity "go.temporal.io/sdk/activity"
	client "go.temporal.io/sdk/client"
	temporal "go.temporal.io/sdk/temporal"
	worker "go.temporal.io/sdk/worker"
	workflow "go.temporal.io/sdk/workflow"
	anypb "google.golang.org/protobuf/types/known/anypb"
	durationpb "google.golang.org/protobuf/types/known/durationpb"
	"time"
)

// OptionalOptions is used to configure test.opaque.Optional xns activity registration
type OptionalOptions struct {
	// errorConverter is used to customize error
	errorConverter func(error) error
	// filter is used to filter xns activity registrations. It receives as
	// input the original activity name, and should return one of the following:
	// 1. the original activity name, for no changes
	// 2. a modified activity name, to override the original activity name
	// 3. an empty string, to skip registration
	filter func(string) string
}

// NewOptionalOptions initializes a new OptionalOptions value
func NewOptionalOptions() *OptionalOptions {
	return &OptionalOptions{}
}

// WithErrorConverter overrides the default error converter applied to xns activity errors
func (opts *OptionalOptions) WithErrorConverter(errorConverter func(error) error) *OptionalOptions {
	opts.errorConverter = errorConverter
	return opts
}

// Filter is used to filter registered xns activities or customize their name
func (opts *OptionalOptions) WithFilter(filter func(string) string) *OptionalOptions {
	opts.filter = filter
	return opts
}

// convertError is applied to all xns activity errors
func (opts *OptionalOptions) convertError(err error) error {
	if err == nil {
		return nil
	}
	if opts != nil && opts.errorConverter != nil {
		return opts.errorConverter(err)
	}
	return xns.ErrorToApplicationError(err)
}

// filterActivity is used to filter xns activity registrations
func (opts *OptionalOptions) filterActivity(name string) string {
	if opts == nil || opts.filter == nil {
		return name
	}
	return opts.filter(name)
}

// optionalOptions is a reference to the OptionalOptions initialized at registration
var optionalOptions *OptionalOptions

// RegisterOptionalActivities registers test.opaque.Optional cross-namespace activities
func RegisterOptionalActivities(r worker.ActivityRegistry, c opaque.OptionalClient, options ...*OptionalOptions) {
	if optionalOptions == nil && len(options) > 0 && options[0] != nil {
		optionalOptions = options[0]
	}
	a := &optionalActivities{c}
	if name := optionalOptions.filterActivity("test.opaque.Optional.CancelWorkflow"); name != "" {
		r.RegisterActivityWithOptions(a.CancelWorkflow, activity.RegisterOptions{Name: name})
	}
	if name := optionalOptions.filterActivity(opaque.PutOptionalExampleWorkflowName); name != "" {
		r.RegisterActivityWithOptions(a.PutOptionalExample, activity.RegisterOptions{Name: name})
	}
	if name := optionalOptions.filterActivity("test.opaque.Optional.PutOptionalExampleWithSignal"); name != "" {
		r.RegisterActivityWithOptions(a.PutOptionalExampleWithSignal, activity.RegisterOptions{Name: name})
	}
	if name := optionalOptions.filterActivity(opaque.SignalSignalName); name != "" {
		r.RegisterActivityWithOptions(a.Signal, activity.RegisterOptions{Name: name})
	}
}

// PutOptionalExampleWorkflowOptions are used to configure a(n) test.opaque.Optional.PutOptionalExample workflow execution
type PutOptionalExampleWorkflowOptions struct {
	ActivityOptions      *workflow.ActivityOptions
	Detached             bool
	HeartbeatInterval    time.Duration
	ParentClosePolicy    enumsv1.ParentClosePolicy
	StartWorkflowOptions *client.StartWorkflowOptions
}

// NewPutOptionalExampleWorkflowOptions initializes a new PutOptionalExampleWorkflowOptions value
func NewPutOptionalExampleWorkflowOptions() *PutOptionalExampleWorkflowOptions {
	return &PutOptionalExampleWorkflowOptions{}
}

// WithActivityOptions can be used to customize the activity options
func (opts *PutOptionalExampleWorkflowOptions) WithActivityOptions(ao workflow.ActivityOptions) *PutOptionalExampleWorkflowOptions {
	opts.ActivityOptions = &ao
	return opts
}

// WithDetached can be used to start a workflow execution and exit immediately
func (opts *PutOptionalExampleWorkflowOptions) WithDetached(d bool) *PutOptionalExampleWorkflowOptions {
	opts.Detached = d
	return opts
}

// WithHeartbeatInterval can be used to customize the activity heartbeat interval
func (opts *PutOptionalExampleWorkflowOptions) WithHeartbeatInterval(d time.Duration) *PutOptionalExampleWorkflowOptions {
	opts.HeartbeatInterval = d
	return opts
}

// WithParentClosePolicy can be used to customize the cancellation propagation behavior
func (opts *PutOptionalExampleWorkflowOptions) WithParentClosePolicy(policy enumsv1.ParentClosePolicy) *PutOptionalExampleWorkflowOptions {
	opts.ParentClosePolicy = policy
	return opts
}

// WithStartWorkflowOptions can be used to customize the start workflow options
func (opts *PutOptionalExampleWorkflowOptions) WithStartWorkflow(swo client.StartWorkflowOptions) *PutOptionalExampleWorkflowOptions {
	opts.StartWorkflowOptions = &swo
	return opts
}

// PutOptionalExampleRun provides a handle to a test.opaque.Optional.PutOptionalExample workflow execution
type PutOptionalExampleRun interface {
	// Cancel cancels the workflow
	Cancel(workflow.Context) error

	// Future returns the inner workflow.Future
	Future() workflow.Future

	// Get returns the inner workflow.Future
	Get(workflow.Context) (*opaque.OptionalExample, error)

	// ID returns the workflow id
	ID() string

	// Signal executes a(n) test.opaque.Optional.Signal signal and blocks until completion
	Signal(workflow.Context, *opaque.OptionalExample, ...*SignalSignalOptions) error

	// SignalAsync executes a(n) test.opaque.Optional.Signal signal and returns a handle to the underlying activity
	SignalAsync(workflow.Context, *opaque.OptionalExample, ...*SignalSignalOptions) (SignalSignalHandle, error)
}

// putOptionalExampleRun provides a(n) PutOptionalExampleRun implementation
type putOptionalExampleRun struct {
	cancel func()
	future workflow.Future
	id     string
}

// Cancel the underlying workflow execution
func (r *putOptionalExampleRun) Cancel(ctx workflow.Context) error {
	if r.cancel != nil {
		r.cancel()
		if _, err := r.Get(ctx); err != nil && !errors.Is(err, workflow.ErrCanceled) {
			return err
		}
		return nil
	}
	return CancelOptionalWorkflow(ctx, r.id, "")
}

// Future returns the underlying activity future
func (r *putOptionalExampleRun) Future() workflow.Future {
	return r.future
}

// Get blocks on activity completion and returns the underlying workflow result
func (r *putOptionalExampleRun) Get(ctx workflow.Context) (*opaque.OptionalExample, error) {
	var resp opaque.OptionalExample
	if err := r.future.Get(ctx, &resp); err != nil {
		return nil, err
	}
	return &resp, nil
}

// ID returns the underlying workflow id
func (r *putOptionalExampleRun) ID() string {
	return r.id
}

// Signal executes a(n) test.opaque.Optional.Signal signal and blocks until the underlying activity completes
func (r *putOptionalExampleRun) Signal(ctx workflow.Context, req *opaque.OptionalExample, opts ...*SignalSignalOptions) error {
	return Signal(ctx, r.ID(), "", req, opts...)
}

// SignalAsync executes a(n) test.opaque.Optional.Signal signal and returns a handle to the underlying activity
func (r *putOptionalExampleRun) SignalAsync(ctx workflow.Context, req *opaque.OptionalExample, opts ...*SignalSignalOptions) (SignalSignalHandle, error) {
	return SignalAsync(ctx, r.ID(), "", req, opts...)
}

// PutOptionalExample executes a(n) test.opaque.Optional.PutOptionalExample workflow and blocks until error or response is received
func PutOptionalExample(ctx workflow.Context, req *opaque.OptionalExample, opts ...*PutOptionalExampleWorkflowOptions) (*opaque.OptionalExample, error) {
	run, err := PutOptionalExampleAsync(ctx, req, opts...)
	if err != nil {
		return nil, err
	}
	return run.Get(ctx)
}

// PutOptionalExampleAsync executes a(n) test.opaque.Optional.PutOptionalExample workflow and returns a handle to the underlying activity
func PutOptionalExampleAsync(ctx workflow.Context, req *opaque.OptionalExample, opts ...*PutOptionalExampleWorkflowOptions) (PutOptionalExampleRun, error) {
	activityName := optionalOptions.filterActivity(opaque.PutOptionalExampleWorkflowName)
	if activityName == "" {
		return nil, temporal.NewNonRetryableApplicationError(
			fmt.Sprintf("no activity registered for %s", opaque.PutOptionalExampleWorkflowName),
			"Unimplemented",
			nil,
		)
	}

	opt := &PutOptionalExampleWorkflowOptions{}
	if len(opts) > 0 && opts[0] != nil {
		opt = opts[0]
	}
	if opt.HeartbeatInterval == 0 {
		opt.HeartbeatInterval = time.Second * 30
	}

	// configure activity options
	ao := workflow.GetActivityOptions(ctx)
	if opt.ActivityOptions != nil {
		ao = *opt.ActivityOptions
	}
	if ao.HeartbeatTimeout == 0 {
		ao.HeartbeatTimeout = opt.HeartbeatInterval * 2
	}
	// WaitForCancellation must be set otherwise the underlying workflow is not guaranteed to be canceled
	ao.WaitForCancellation = true

	if ao.StartToCloseTimeout == 0 && ao.ScheduleToCloseTimeout == 0 {
		ao.ScheduleToCloseTimeout = 86400000000000 // 1 day
	}
	ctx = workflow.WithActivityOptions(ctx, ao)

	// configure start workflow options
	wo := client.StartWorkflowOptions{}
	if opt.StartWorkflowOptions != nil {
		wo = *opt.StartWorkflowOptions
	}
	if wo.ID == "" {
		if err := workflow.SideEffect(ctx, func(ctx workflow.Context) any {
			id, err := uuid.NewRandom()
			if err != nil {
				workflow.GetLogger(ctx).Error("error generating workflow id", "error", err)
				return nil
			}
			return id
		}).Get(&wo.ID); err != nil {
			return nil, err
		}
	}
	if wo.ID == "" {
		return nil, temporal.NewNonRetryableApplicationError("workflow id is required", "InvalidArgument", nil)
	}

	// marshal start workflow options protobuf message
	swo, err := xns.MarshalStartWorkflowOptions(wo)
	if err != nil {
		return nil, fmt.Errorf("error marshalling start workflow options: %w", err)
	}

	// marshal workflow request protobuf message
	wreq, err := anypb.New(req)
	if err != nil {
		return nil, fmt.Errorf("error marshalling workflow request: %w", err)
	}

	var parentClosePolicy temporalv1.ParentClosePolicy
	switch opt.ParentClosePolicy {
	case enumsv1.PARENT_CLOSE_POLICY_ABANDON:
		parentClosePolicy = temporalv1.ParentClosePolicy_PARENT_CLOSE_POLICY_ABANDON
	case enumsv1.PARENT_CLOSE_POLICY_REQUEST_CANCEL:
		parentClosePolicy = temporalv1.ParentClosePolicy_PARENT_CLOSE_POLICY_REQUEST_CANCEL
	case enumsv1.PARENT_CLOSE_POLICY_TERMINATE:
		parentClosePolicy = temporalv1.ParentClosePolicy_PARENT_CLOSE_POLICY_TERMINATE
	}

	ctx, cancel := workflow.WithCancel(ctx)
	return &putOptionalExampleRun{
		cancel: cancel,
		id:     wo.ID,
		future: workflow.ExecuteActivity(ctx, activityName, &xnsv1.WorkflowRequest{
			Detached:             opt.Detached,
			HeartbeatInterval:    durationpb.New(opt.HeartbeatInterval),
			ParentClosePolicy:    parentClosePolicy,
			Request:              wreq,
			StartWorkflowOptions: swo,
		}),
	}, nil
}

// PutOptionalExampleWithSignal sends a(n) test.opaque.Optional.Signal signal to a test.opaque.Optional.PutOptionalExample workflow, starting it if necessary, and blocks until the workflow completes
func PutOptionalExampleWithSignal(ctx workflow.Context, req *opaque.OptionalExample, signal *opaque.OptionalExample, opts ...*PutOptionalExampleWorkflowOptions) (*opaque.OptionalExample, error) {
	run, err := PutOptionalExampleWithSignalAsync(ctx, req, signal, opts...)
	if err != nil {
		return nil, err
	}
	return run.Get(ctx)
}

// PutOptionalExampleWithSignalAsync sends a(n) test.opaque.Optional.Signal signal to a(n) test.opaque.Optional.PutOptionalExample workflow, starting it if necessary, and returns a handle to the underlying activity
func PutOptionalExampleWithSignalAsync(ctx workflow.Context, req *opaque.OptionalExample, signal *opaque.OptionalExample, opts ...*PutOptionalExampleWorkflowOptions) (PutOptionalExampleRun, error) {
	activityName := optionalOptions.filterActivity("test.opaque.Optional.PutOptionalExampleWithSignal")
	if activityName == "" {
		return nil, temporal.NewNonRetryableApplicationError(
			fmt.Sprintf("no activity registered for %s", "test.opaque.Optional.PutOptionalExampleWithSignal"),
			"Unimplemented",
			nil,
		)
	}

	opt := &PutOptionalExampleWorkflowOptions{}
	if len(opts) > 0 && opts[0] != nil {
		opt = opts[0]
	}
	if opt.HeartbeatInterval == 0 {
		opt.HeartbeatInterval = time.Second * 30
	}

	// configure activity options
	ao := workflow.GetActivityOptions(ctx)
	if opt.ActivityOptions != nil {
		ao = *opt.ActivityOptions
	}
	if ao.HeartbeatTimeout == 0 {
		ao.HeartbeatTimeout = opt.HeartbeatInterval * 2
	}
	// WaitForCancellation must be set otherwise the underlying workflow is not guaranteed to be canceled
	ao.WaitForCancellation = true

	if ao.StartToCloseTimeout == 0 && ao.ScheduleToCloseTimeout == 0 {
		ao.ScheduleToCloseTimeout = 86400000000000 // 1 day
	}
	ctx = workflow.WithActivityOptions(ctx, ao)

	// configure start workflow options
	wo := client.StartWorkflowOptions{}
	if opt.StartWorkflowOptions != nil {
		wo = *opt.StartWorkflowOptions
	}
	if wo.ID == "" {
		if err := workflow.SideEffect(ctx, func(ctx workflow.Context) any {
			id, err := uuid.NewRandom()
			if err != nil {
				workflow.GetLogger(ctx).Error("error generating workflow id", "error", err)
				return nil
			}
			return id
		}).Get(&wo.ID); err != nil {
			return nil, err
		}
	}
	if wo.ID == "" {
		return nil, temporal.NewNonRetryableApplicationError("workflow id is required", "InvalidArgument", nil)
	}

	// marshal start workflow options protobuf message
	swo, err := xns.MarshalStartWorkflowOptions(wo)
	if err != nil {
		return nil, fmt.Errorf("error marshalling start workflow options: %w", err)
	}

	// marshal workflow request protobuf message
	wreq, err := anypb.New(req)
	if err != nil {
		return nil, fmt.Errorf("error marshalling workflow request: %w", err)
	}

	// marshal signal request protobuf message
	wsignal, err := anypb.New(signal)
	if err != nil {
		return nil, fmt.Errorf("error marshalling signal request: %w", err)
	}

	var parentClosePolicy temporalv1.ParentClosePolicy
	switch opt.ParentClosePolicy {
	case enumsv1.PARENT_CLOSE_POLICY_ABANDON:
		parentClosePolicy = temporalv1.ParentClosePolicy_PARENT_CLOSE_POLICY_ABANDON
	case enumsv1.PARENT_CLOSE_POLICY_REQUEST_CANCEL:
		parentClosePolicy = temporalv1.ParentClosePolicy_PARENT_CLOSE_POLICY_REQUEST_CANCEL
	case enumsv1.PARENT_CLOSE_POLICY_TERMINATE:
		parentClosePolicy = temporalv1.ParentClosePolicy_PARENT_CLOSE_POLICY_TERMINATE
	}

	ctx, cancel := workflow.WithCancel(ctx)
	return &putOptionalExampleRun{
		cancel: cancel,
		id:     wo.ID,
		future: workflow.ExecuteActivity(ctx, activityName, &xnsv1.WorkflowRequest{
			Detached:             opt.Detached,
			HeartbeatInterval:    durationpb.New(opt.HeartbeatInterval),
			ParentClosePolicy:    parentClosePolicy,
			Request:              wreq,
			Signal:               wsignal,
			StartWorkflowOptions: swo,
		}),
	}, nil
}

// SignalSignalOptions are used to configure a(n) test.opaque.Optional.Signal signal execution
type SignalSignalOptions struct {
	ActivityOptions   *workflow.ActivityOptions
	HeartbeatInterval time.Duration
}

// NewSignalSignalOptions initializes a new SignalSignalOptions value
func NewSignalSignalOptions() *SignalSignalOptions {
	return &SignalSignalOptions{}
}

// WithActivityOptions can be used to customize the activity options
func (opts *SignalSignalOptions) WithActivityOptions(ao workflow.ActivityOptions) *SignalSignalOptions {
	opts.ActivityOptions = &ao
	return opts
}

// WithHeartbeatInterval can be used to customize the activity heartbeat interval
func (opts *SignalSignalOptions) WithHeartbeatInterval(d time.Duration) *SignalSignalOptions {
	opts.HeartbeatInterval = d
	return opts
}

// SignalSignalHandle provides a handle for a test.opaque.Optional.Signal signal activity
type SignalSignalHandle interface {
	// Cancel cancels the workflow
	Cancel(workflow.Context) error
	// Future returns the inner workflow.Future
	Future() workflow.Future
	// Get returns the inner workflow.Future
	Get(workflow.Context) error
}

// signalSignalHandle provides a(n) SignalQueryHandle implementation
type signalSignalHandle struct {
	cancel func()
	future workflow.Future
}

// Cancel the underlying signal activity
func (r *signalSignalHandle) Cancel(ctx workflow.Context) error {
	r.cancel()
	if err := r.Get(ctx); err != nil && !errors.Is(err, workflow.ErrCanceled) {
		return err
	}
	return nil
}

// Future returns the underlying activity future
func (r *signalSignalHandle) Future() workflow.Future {
	return r.future
}

// Get blocks on activity completion
func (r *signalSignalHandle) Get(ctx workflow.Context) error {
	return r.future.Get(ctx, nil)
}

// Signal executes a(n) test.opaque.Optional.Signal signal
func Signal(ctx workflow.Context, workflowID string, runID string, req *opaque.OptionalExample, opts ...*SignalSignalOptions) error {
	handle, err := SignalAsync(ctx, workflowID, runID, req, opts...)
	if err != nil {
		return err
	}
	return handle.Get(ctx)
}

// SignalAsync executes a(n) test.opaque.Optional.Signal signal
func SignalAsync(ctx workflow.Context, workflowID string, runID string, req *opaque.OptionalExample, opts ...*SignalSignalOptions) (SignalSignalHandle, error) {
	activityName := optionalOptions.filterActivity(opaque.SignalSignalName)
	if activityName == "" {
		return nil, temporal.NewNonRetryableApplicationError(
			fmt.Sprintf("no activity registered for %s", opaque.SignalSignalName),
			"Unimplemented",
			nil,
		)
	}

	opt := &SignalSignalOptions{}
	if len(opts) > 0 && opts[0] != nil {
		opt = opts[0]
	}

	if opt.HeartbeatInterval == 0 {
		opt.HeartbeatInterval = time.Second * 30
	}

	// configure activity options
	ao := workflow.GetActivityOptions(ctx)
	if opt.ActivityOptions != nil {
		ao = *opt.ActivityOptions
	}
	if ao.HeartbeatTimeout == 0 {
		ao.HeartbeatTimeout = opt.HeartbeatInterval * 2
	}
	// WaitForCancellation must be set otherwise the underlying workflow is not guaranteed to be canceled
	ao.WaitForCancellation = true

	if ao.StartToCloseTimeout == 0 && ao.ScheduleToCloseTimeout == 0 {
		ao.ScheduleToCloseTimeout = 60000000000 // 1 minute
	}
	ctx = workflow.WithActivityOptions(ctx, ao)

	// marshal workflow request
	wreq, err := anypb.New(req)
	if err != nil {
		return nil, fmt.Errorf("error marshalling workflow request: %w", err)
	}

	ctx, cancel := workflow.WithCancel(ctx)
	return &signalSignalHandle{
		cancel: cancel,
		future: workflow.ExecuteActivity(ctx, activityName, &xnsv1.SignalRequest{
			HeartbeatInterval: durationpb.New(opt.HeartbeatInterval),
			WorkflowId:        workflowID,
			RunId:             runID,
			Request:           wreq,
		}),
	}, nil
}

// CancelOptionalWorkflow cancels an existing workflow
func CancelOptionalWorkflow(ctx workflow.Context, workflowID string, runID string) error {
	return CancelOptionalWorkflowAsync(ctx, workflowID, runID).Get(ctx, nil)
}

// CancelOptionalWorkflowAsync cancels an existing workflow
func CancelOptionalWorkflowAsync(ctx workflow.Context, workflowID string, runID string) workflow.Future {
	activityName := optionalOptions.filterActivity("test.opaque.Optional.CancelWorkflow")
	if activityName == "" {
		f, s := workflow.NewFuture(ctx)
		s.SetError(temporal.NewNonRetryableApplicationError(
			"no activity registered for test.opaque.Optional.CancelWorkflow",
			"Unimplemented",
			nil,
		))
		return f
	}
	ao := workflow.GetActivityOptions(ctx)
	if ao.StartToCloseTimeout == 0 && ao.ScheduleToCloseTimeout == 0 {
		ao.StartToCloseTimeout = time.Minute
	}
	ctx = workflow.WithActivityOptions(ctx, ao)
	return workflow.ExecuteActivity(ctx, activityName, workflowID, runID)
}

// optionalActivities provides activities that can be used to interact with a(n) Optional service's workflow, queries, signals, and updates across namespaces
type optionalActivities struct {
	client opaque.OptionalClient
}

// CancelWorkflow cancels an existing workflow execution
func (a *optionalActivities) CancelWorkflow(ctx context.Context, workflowID string, runID string) error {
	return a.client.CancelWorkflow(ctx, workflowID, runID)
}

// PutOptionalExample executes a(n) test.opaque.Optional.PutOptionalExample workflow via an activity
func (a *optionalActivities) PutOptionalExample(ctx context.Context, input *xnsv1.WorkflowRequest) (resp *opaque.OptionalExample, err error) {
	// unmarshal workflow request
	var req opaque.OptionalExample
	if err := input.Request.UnmarshalTo(&req); err != nil {
		return nil, optionalOptions.convertError(temporal.NewNonRetryableApplicationError(
			fmt.Sprintf("error unmarshalling workflow request of type %s as github.com/cludden/protoc-gen-go-temporal/gen/test/opaque.OptionalExample", input.Request.GetTypeUrl()),
			"InvalidArgument",
			err,
		))
	}

	// initialize workflow execution
	var run opaque.PutOptionalExampleRun
	run, err = a.client.PutOptionalExampleAsync(ctx, &req, opaque.NewPutOptionalExampleOptions().WithStartWorkflowOptions(
		xns.UnmarshalStartWorkflowOptions(input.GetStartWorkflowOptions()),
	))
	if err != nil {
		return nil, optionalOptions.convertError(err)
	}

	// exit early if detached enabled
	if input.GetDetached() {
		return nil, nil
	}

	// otherwise, wait for execution to complete in child goroutine
	doneCh := make(chan struct{})
	go func() {
		resp, err = run.Get(ctx)
		close(doneCh)
	}()

	heartbeatInterval := input.GetHeartbeatInterval().AsDuration()
	if heartbeatInterval == 0 {
		heartbeatInterval = time.Second * 30
	}

	// heartbeat activity while waiting for workflow execution to complete
	for {
		select {
		// send heartbeats periodically
		case <-time.After(heartbeatInterval):
			activity.RecordHeartbeat(ctx, run.ID())

		// return retryable error on worker close
		case <-activity.GetWorkerStopChannel(ctx):
			return nil, temporal.NewApplicationError("worker is stopping", "WorkerStopped")

		// catch parent activity context cancellation. in most cases, this should indicate a
		// server-sent cancellation, but there's a non-zero possibility that this cancellation
		// is received due to the worker stopping, prior to detecting the closing of the worker
		// stop channel. to give us an opportunity to detect a cancellation stemming from the
		// worker closing, we again check to see if the worker stop channel is closed before
		// propagating the cancellation
		case <-ctx.Done():
			select {
			case <-activity.GetWorkerStopChannel(ctx):
				return nil, temporal.NewApplicationError("worker is stopping", "WorkerStopped")
			default:
				parentClosePolicy := input.GetParentClosePolicy()
				if parentClosePolicy == temporalv1.ParentClosePolicy_PARENT_CLOSE_POLICY_REQUEST_CANCEL || parentClosePolicy == temporalv1.ParentClosePolicy_PARENT_CLOSE_POLICY_TERMINATE {
					disconnectedCtx, cancel := context.WithTimeout(context.Background(), time.Minute)
					defer cancel()
					if parentClosePolicy == temporalv1.ParentClosePolicy_PARENT_CLOSE_POLICY_REQUEST_CANCEL {
						err = run.Cancel(disconnectedCtx)
					} else {
						err = run.Terminate(disconnectedCtx, "xns activity cancellation received", "error", ctx.Err())
					}
					if err != nil {
						return nil, optionalOptions.convertError(err)
					}
				}
				return nil, optionalOptions.convertError(temporal.NewCanceledError(ctx.Err().Error()))
			}

		// handle workflow completion
		case <-doneCh:
			return resp, optionalOptions.convertError(err)
		}
	}
}

// PutOptionalExampleWithSignal sends a(n) test.opaque.Optional.Signal signal to a(n) test.opaque.Optional.PutOptionalExample workflow via an activity
func (a *optionalActivities) PutOptionalExampleWithSignal(ctx context.Context, input *xnsv1.WorkflowRequest) (resp *opaque.OptionalExample, err error) {
	// unmarshal workflow request
	var req opaque.OptionalExample
	if err := input.Request.UnmarshalTo(&req); err != nil {
		return nil, optionalOptions.convertError(temporal.NewNonRetryableApplicationError(
			fmt.Sprintf("error unmarshalling workflow request of type %s as github.com/cludden/protoc-gen-go-temporal/gen/test/opaque.OptionalExample", input.Request.GetTypeUrl()),
			"InvalidArgument",
			err,
		))
	}

	// unmarshal signal request
	var signal opaque.OptionalExample
	if err := input.Signal.UnmarshalTo(&signal); err != nil {
		return nil, optionalOptions.convertError(temporal.NewNonRetryableApplicationError(
			fmt.Sprintf("error unmarshalling signal request of type %s as github.com/cludden/protoc-gen-go-temporal/gen/test/opaque.OptionalExample", input.Signal.GetTypeUrl()),
			"InvalidArgument",
			err,
		))
	}

	// initialize workflow execution
	var run opaque.PutOptionalExampleRun
	run, err = a.client.PutOptionalExampleWithSignalAsync(ctx, &req, &signal, opaque.NewPutOptionalExampleOptions().WithStartWorkflowOptions(
		xns.UnmarshalStartWorkflowOptions(input.GetStartWorkflowOptions()),
	))
	if err != nil {
		return nil, optionalOptions.convertError(err)
	}

	// exit early if detached enabled
	if input.GetDetached() {
		return nil, nil
	}

	// otherwise, wait for execution to complete in child goroutine
	doneCh := make(chan struct{})
	go func() {
		resp, err = run.Get(ctx)
		close(doneCh)
	}()

	heartbeatInterval := input.GetHeartbeatInterval().AsDuration()
	if heartbeatInterval == 0 {
		heartbeatInterval = time.Second * 30
	}

	// heartbeat activity while waiting for workflow execution to complete
	for {
		select {
		// send heartbeats periodically
		case <-time.After(heartbeatInterval):
			activity.RecordHeartbeat(ctx, run.ID())

		// return retryable error on worker close
		case <-activity.GetWorkerStopChannel(ctx):
			return nil, temporal.NewApplicationError("worker is stopping", "WorkerStopped")

		// catch parent activity context cancellation. in most cases, this should indicate a
		// server-sent cancellation, but there's a non-zero possibility that this cancellation
		// is received due to the worker stopping, prior to detecting the closing of the worker
		// stop channel. to give us an opportunity to detect a cancellation stemming from the
		// worker closing, we again check to see if the worker stop channel is closed before
		// propagating the cancellation
		case <-ctx.Done():
			select {
			case <-activity.GetWorkerStopChannel(ctx):
				return nil, temporal.NewApplicationError("worker is stopping", "WorkerStopped")
			default:
				parentClosePolicy := input.GetParentClosePolicy()
				if parentClosePolicy == temporalv1.ParentClosePolicy_PARENT_CLOSE_POLICY_REQUEST_CANCEL || parentClosePolicy == temporalv1.ParentClosePolicy_PARENT_CLOSE_POLICY_TERMINATE {
					disconnectedCtx, cancel := context.WithTimeout(context.Background(), time.Minute)
					defer cancel()
					if parentClosePolicy == temporalv1.ParentClosePolicy_PARENT_CLOSE_POLICY_REQUEST_CANCEL {
						err = run.Cancel(disconnectedCtx)
					} else {
						err = run.Terminate(disconnectedCtx, "xns activity cancellation received", "error", ctx.Err())
					}
					if err != nil {
						return nil, optionalOptions.convertError(err)
					}
				}
				return nil, optionalOptions.convertError(temporal.NewCanceledError(ctx.Err().Error()))
			}

		// handle workflow completion
		case <-doneCh:
			return resp, optionalOptions.convertError(err)
		}
	}
}

// Signal executes a(n) test.opaque.Optional.Signal signal via an activity
func (a *optionalActivities) Signal(ctx context.Context, input *xnsv1.SignalRequest) (err error) {
	// unmarshal signal request
	var req opaque.OptionalExample
	if err := input.Request.UnmarshalTo(&req); err != nil {
		return optionalOptions.convertError(temporal.NewNonRetryableApplicationError(
			fmt.Sprintf("error unmarshalling signal request of type %s as github.com/cludden/protoc-gen-go-temporal/gen/test/opaque.OptionalExample", input.Request.GetTypeUrl()),
			"InvalidArgument",
			err,
		))
	}
	// execute signal in child goroutine
	doneCh := make(chan struct{})
	go func() {
		err = a.client.Signal(ctx, input.GetWorkflowId(), input.GetRunId(), &req)
		close(doneCh)
	}()

	heartbeatInterval := input.GetHeartbeatInterval().AsDuration()
	if heartbeatInterval == 0 {
		heartbeatInterval = time.Second * 10
	}

	// heartbeat activity while waiting for signal to complete
	for {
		select {
		case <-time.After(heartbeatInterval):
			activity.RecordHeartbeat(ctx)
		case <-ctx.Done():
			optionalOptions.convertError(ctx.Err())
		case <-doneCh:
			return optionalOptions.convertError(err)
		}
	}
}
