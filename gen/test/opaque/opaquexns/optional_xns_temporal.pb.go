...
func GetPutOptionalExample(ctx workflow.Context, workflowID string, runID string) (out *opaque.OptionalExample, err error) {
	out, err = GetPutOptionalExampleAsync(ctx, workflowID, runID).Get(ctx)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func GetPutOptionalExampleAsync(ctx workflow.Context, workflowID string, runID string) PutOptionalExampleRun {
...
func (a *optionalActivities) GetPutOptionalExample(ctx context.Context, input *xnsv1.GetWorkflowRequest) (out *opaque.OptionalExample, err error) {
	...
	go func() {
		defer close(done)
		out, err = a.client.GetPutOptionalExample(ctx, input.GetWorkflowId(), input.GetRunId()).Get(ctx)
	}()
	for {
		select {
		...
		case <-activity.GetWorkerStopChannel(ctx):
			return nil, optionalOptions.convertError(temporal.NewApplicationError("worker is stopping", "WorkerStopping"))
		case <-ctx.Done():
			return nil, optionalOptions.convertError(err)
		}
	}
