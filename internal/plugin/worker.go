	...
	f.Type().Id(typeName).InterfaceFunc(func(methods *g.Group) {
		...
		for _, queryOpts := range opts.GetQuery() {
			...
			if handler.Comments.Leading.String() != "" {
				...
				methods.Commentf("%s implements a(n) %s query handler", svc.methods[query].GoName, handler.Desc.FullName())
			}
		...
		for _, updateOpts := range opts.GetUpdate() {
			...
			if handler.Comments.Leading.String() != "" {
				...
				methods.Commentf("%s implements a(n) %s update handler", svc.methods[update].GoName, handler.Desc.FullName())
			}
	...
	f.Type().Id(typeName).InterfaceFunc(func(methods *g.Group) {
		for _, workflow := range svc.workflowsOrdered {
			...
			if handler.Comments.Leading.String() != "" {
				...
				f.Commentf("%s initializes a new a(n) %s implementation", svc.methods[workflow].GoName, svc.toCamel("%sWorkflow", workflow))
			}
			methods.
