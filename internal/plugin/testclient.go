...
func (m *Manifest) genTestClientWorkflowRunImplGetMethod(f *j.File, workflow protoreflect.FullName) {
	...
	f.Func().
		...
		BlockFunc(func(fn *j.Group) {
			// execute workflow
			fn.Id("r").Dot("env").Dot("ExecuteWorkflow").CallFunc(func(args *j.Group) {
			...
			// ensure completed
			fn.If(j.Op("!").Id("r").Dot("env").Dot("IsWorkflowCompleted").Call()).Block(
				j.ReturnFunc(func(returnVals *j.Group) {
					if hasOutput {
						returnVals.Nil()
					}
					returnVals.Qual("errors", "New").Call(j.Lit("workflow in progress"))
				}),
			)
			// return workflow error if applicable
			fn.If(j.Err().Op(":=").Id("r").Dot("env").Dot("GetWorkflowError").Call(), j.Err().Op("!=").Nil()).Block(
				j.ReturnFunc(func(returnVals *j.Group) {
					if hasOutput {
						returnVals.Nil()
					}
			...
			if hasOutput {
				fn.Var().Id("result").Qual(string(method.Output.GoIdent.GoImportPath), m.getMessageName(method.Output))
				fn.If(j.Err().Op(":=").Id("r").Dot("env").Dot("GetWorkflowResult").Call(j.Op("&").Id("result")), j.Err().Op("!=").Nil()).Block(
					j.Return(j.Nil(), j.Err()),
				)
				fn.Return(j.Op("&").Id("result"), j.Nil())
			} else {
				fn.Return(j.Nil())
			}
		})
