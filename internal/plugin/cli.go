...
func (svc *Service) renderCLI(f *g.File) {
	svc.genOptions(f)
	svc.genOption(f)
	svc.genOptionWithClientForCommand(f)
	svc.genNewCommands(f)
	...
	for _, query := range svc.queriesOrdered {
		...
		svc.genUnmarshalMessage(f, svc.methods[query].Input)
	}
	...
	for _, signal := range svc.signalsOrdered {
		...
		svc.genUnmarshalMessage(f, svc.methods[signal].Input)
	}
	...
	for _, workflow := range svc.workflowsOrdered {
		...
		svc.genUnmarshalMessage(f, svc.methods[workflow].Input)
	}
}

// genFlagForField generates a cli flag for a message field
func (svc *Service) genFlagForField(flags *g.Group, field *protogen.Field, category string) {
	name := field.GoName
...
// genNewCommands generates a New<Service>Commands contructor function
func (svc *Service) genNewCommands(f *g.File) {
	...
	f.Func().Id("NewCommands").
		...
		Block(
			...
			g.Id("commands").Op(":=").Index().Op("*").Qual(cliPkg, "Command").CustomFunc(g.Options{
				...
				for _, query := range svc.queriesOrdered {
					svc.genQuery(cmds, query)
				}

				// generate client signal methods
				for _, signal := range svc.signalsOrdered {
					svc.genSignal(cmds, signal)
				}

				// // generate client update methods
				// for _, update := range svc.updatesOrdered {
				// 	svc.genUpdate(cmds, update)
				// 	svc.genUpdateAsync(cmds, update)
				// }

				// generate client workflow methods
				for _, workflow := range svc.workflowsOrdered {
					svc.genWorkflowCommand(cmds, workflow)
					for _, signal := range svc.workflows[workflow].GetSignal() {
						if !signal.GetStart() {
							continue
						}
						svc.genWorkflowWithSignalCommand(cmds, workflow, signal.GetRef())
					}
				}
			}),
...
// genOption generates an CLIOption interface
func (svc *Service) genOption(f *g.File) {
	f.Commentf("Option describes a CLI option")
	f.Type().Id("CLIOption").Func().Params(g.Op("*").Id("CLIOptions")).Error()
}

// genOptionWithClientForCommand generates a WithClientForCommand option
func (svc *Service) genOptionWithClientForCommand(f *g.File) {
	...
	f.Func().Id("WithClientForCommand").
		...
		Id("CLIOption").
		Block(genOptionReturn(func(fn *g.Group) {
...
func genOptionReturn(fn func(*g.Group)) g.Code {
...
// genOptions generates a CLIOptions struct
func (svc *Service) genOptions(f *g.File) {
...
// genQuery generates a <Query> command
func (svc *Service) genQuery(cmds *g.Group, query string) {
	...
	cmds.CustomFunc(multiLineValues, func(cmd *g.Group) {
		...
		cmd.Id("Flags").Op(":").Index().Qual(cliPkg, "Flag").CustomFunc(multiLineValues, func(flags *g.Group) {
			...
			if hasInput {
				...
				for _, field := range method.Input.Fields {
					svc.genFlagForField(flags, field, "")
				}
			}
		})
		cmd.Id("Action").Op(":").Func().Params(g.Id("cmd").Op("*").Qual(cliPkg, "Context")).Error().BlockFunc(func(fn *g.Group) {
			...
			exec.Id("client").Dot(fmt.Sprintf("Query%s", query)).CallFunc(func(args *g.Group) {
...
// genSignal generates a <Signal> command
func (svc *Service) genSignal(cmds *g.Group, signal string) {
	...
	cmds.CustomFunc(multiLineValues, func(cmd *g.Group) {
		...
		cmd.Id("Action").Op(":").Func().Params(g.Id("cmd").Op("*").Qual(cliPkg, "Context")).Error().BlockFunc(func(fn *g.Group) {
			...
			fn.If(
				g.Err().Op(":=").Id("client").Dot(fmt.Sprintf("Signal%s", signal)).CallFunc(func(args *g.Group) {
					args.Id("cmd").Dot("Context")
					args.Id("cmd").Dot("String").Call(g.Lit("workflow-id"))
					args.Id("cmd").Dot("String").Call(g.Lit("run-id"))
					if hasInput {
...
// genUnmarshalMessage generates an unmarshalCliFlagsTo<Message> function
func (svc *Service) genUnmarshalMessage(f *g.File, msg *protogen.Message) {
...
// genWorkflowCommand generates a <Workflow> command
func (svc *Service) genWorkflowCommand(f *g.Group, workflow string) {
	...
	if desc != "" {
		...
		desc = fmt.Sprintf("%s executes a %s workflow and blocks until error or response received", workflow, workflow)
	}
	f.Comment(desc)
	f.CustomFunc(multiLineValues, func(cmd *g.Group) {
		...
		cmd.Id("Action").Op(":").Func().Params(g.Id("cmd").Op("*").Qual(cliPkg, "Context")).Error().BlockFunc(func(fn *g.Group) {
			...
			// execute operation
			fn.List(g.Id("run"), g.Err()).Op(":=").Id("client").Dot(fmt.Sprintf("Execute%s", workflow)).CallFunc(func(args *g.Group) {
				args.Id("cmd").Dot("Context")
				args.Nil()
				if hasInput {
					args.Id("req")
...
// genWorkflowWithStartCommand generates a <Workflow>-with-<Signal> command
func (svc *Service) genWorkflowWithSignalCommand(cmds *g.Group, workflow, signal string) {
	method := svc.methods[workflow]
	hasInput := !isEmpty(method.Input)
	hasOutput := !isEmpty(method.Output)
	...
	desc := fmt.Sprintf("sends a %s signal to a %s worfklow, starting it if it doesn't exist", signal, workflow)
	...
	cmds.CustomFunc(multiLineValues, func(cmd *g.Group) {
		...
		cmd.Id("Flags").Op(":").Index().Qual(cliPkg, "Flag").CustomFunc(multiLineValues, func(flags *g.Group) {
			...
			if hasInput {
				...
				// add request flags
				for _, field := range method.Input.Fields {
					svc.genFlagForField(flags, field, category)
				}
			}
			if hasSignalInput {
				var category string
		...
		cmd.Id("Action").Op(":").Func().Params(g.Id("cmd").Op("*").Qual(cliPkg, "Context")).Error().BlockFunc(func(fn *g.Group) {
			...
			// execute operation
			fn.List(g.Id("run"), g.Err()).Op(":=").Id("client").Dot(fmt.Sprintf("Start%sWith%s", workflow, signal)).CallFunc(func(args *g.Group) {
				args.Id("cmd").Dot("Context")
				args.Nil()
				if hasInput {
					args.Id("req")
