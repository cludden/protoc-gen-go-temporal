	...
	f.Func().
		...
		BlockFunc(func(fn *j.Group) {
			...
			fn.List(j.Id("handle"), j.Err()).Op(":=").Id("c").Dot(m.toCamel("%sAsync", update)).CallFunc(func(args *j.Group) {
				...
				args.Id("o").Dot("WithWaitPolicy").Call(j.Qual(clientPkg, "WorkflowUpdateStageCompleted"))
			})
			fn.If(j.Err().Op("!=").Nil()).Block(
				j.ReturnFunc(func(returnVals *j.Group) {
					if hasOutput {
...
func (m *Manifest) genClientImplUpdateMethodAsync(f *j.File, update protoreflect.FullName) {
	...
	f.Func().
		...
		BlockFunc(func(fn *j.Group) {
			...
			fn.List(j.Id("handle"), j.Err()).Op(":=").Id("c").Dot("client").Dot("UpdateWorkflow").Call(j.Id("ctx"), j.Op("*").Id("options"))
			fn.If(j.Err().Op("!=").Nil()).Block(
				j.Return(j.Nil(), j.Err()),
			)
...
func (m *Manifest) genClientImplWorkflowMethod(f *j.File, workflow protoreflect.FullName) {
	...
	f.Func().
		...
		BlockFunc(func(fn *j.Group) {
			// execute workflow
			fn.Id("run").Op(",").Err().Op(":=").Id("c").Dot(m.toCamel("%sAsync", workflow)).CallFunc(func(args *j.Group) {
				args.Id("ctx")
				if hasInput {
					args.Id("req")
				}
