...
func (svc *Manifest) genClientImplQueryMethod(f *g.File, query protoreflect.FullName) {
	...
	f.Commentf("%s sends a(n) %s query to an existing workflow", svc.methods[query].GoName, svc.fqnForQuery(query))
	f.Func().
		...
		Block(
			g.Var().Id("resp").Id(svc.getMessageName(method.Output)),
			g.If(
...
func (svc *Manifest) genClientImplSignalMethod(f *g.File, signal protoreflect.FullName) {
	...
	f.Commentf("%s sends a(n) %s signal to an existing workflow", svc.methods[signal].GoName, svc.fqnForSignal(signal))
	f.Func().
		Params(g.Id("c").Op("*").Id(clientType)).
...
func (svc *Manifest) genClientImplUpdateMethod(f *g.File, update protoreflect.FullName) {
	...
	f.Commentf("%s sends a(n) %s update to an existing workflow", svc.methods[update].GoName, svc.fqnForUpdate(update))
	f.Func().
		Params(g.Id("c").Op("*").Id(clientType)).
...
func (svc *Manifest) genClientImplWorkflowMethod(f *g.File, workflow protoreflect.FullName) {
	...
	f.Commentf("%s executes a %s workflow and blocks until error or response received", svc.methods[workflow].GoName, svc.fqnForWorkflow(workflow))
	f.Func().
		Params(g.Id("c").Op("*").Id(clientType)).
...
func (svc *Manifest) genClientInterface(f *g.File) {
	...
	f.Type().Id(typeName).InterfaceFunc(func(methods *g.Group) {
		for _, workflow := range svc.workflowsOrdered {
			...
			methods.Id(svc.methods[workflow].GoName).
				ParamsFunc(func(args *g.Group) {
					args.Id("ctx").Qual("context", "Context")
					if hasInput {
						args.Id("req").Op("*").Id(svc.getMessageName(method.Input))
		...
		for _, query := range svc.queriesOrdered {
			...
			methods.Id(svc.methods[query].GoName).
				ParamsFunc(func(args *g.Group) {
					args.Id("ctx").Qual("context", "Context")
					args.Id("workflowID").String()
					args.Id("runID").String()
		...
		for _, signal := range svc.signalsOrdered {
			...
			methods.Id(svc.methods[signal].GoName).
				ParamsFunc(func(args *g.Group) {
					args.Id("ctx").Qual("context", "Context")
					args.Id("workflowID").String()
					args.Id("runID").String()
		...
		for _, update := range svc.updatesOrdered {
			...
			methods.Id(svc.methods[update].GoName).
				ParamsFunc(func(args *g.Group) {
					args.Id("ctx").Qual("context", "Context")
					args.Id("workflowID").String()
					args.Id("runID").String()
			...
			methods.Commentf("%s retrieves a handle to an existing %s update", svc.methods[update].GoName, svc.fqnForUpdate(update))
			methods.Id(methodName).
				ParamsFunc(func(args *g.Group) {
					args.Id("ctx").Qual("context", "Context")
			...
			fn.Go().Func().Params().Block(
				g.For().Block(
					g.If(
						...
						g.Err().Op("!=").Nil().Op("&&").Qual("strings", "Contains").Call(
...
func (svc *Manifest) genClientUpdateWorkflowOptions(fn *g.Group, update protoreflect.FullName) {
	...
	fn.Id("options").Dot("WaitPolicy").Op("=").Op("&").Qual(updatePkg, "WaitPolicy").Custom(multiLineValues,
...
func (svc *Manifest) genClientWorkflowRunImplQueryMethod(f *g.File, workflow protoreflect.FullName, query protoreflect.FullName) {
	...
	f.Commentf("%s executes a(n) %s query", svc.methods[query].GoName, svc.fqnForQuery(query))
	f.Func().
		Params(g.Id("r").Op("*").Id(typeName)).
		Id(svc.methods[query].GoName).
		ParamsFunc(func(args *g.Group) {
...
func (svc *Manifest) genClientWorkflowRunImplSignalMethod(f *g.File, workflow protoreflect.FullName, signal protoreflect.FullName) {
	...
	// generate get method
	f.Commentf("%s sends a(n) %s signal", svc.methods[signal].GoName, svc.fqnForSignal(signal))
	f.Func().
		Params(g.Id("r").Op("*").Id(typeName)).
		Id(svc.methods[signal].GoName).
		ParamsFunc(func(args *g.Group) {
...
func (svc *Manifest) genClientWorkflowRunImplUpdateMethod(f *g.File, workflow protoreflect.FullName, update protoreflect.FullName) {
	...
	// generate get method
	f.Commentf("%s executes a(n) %s update", svc.methods[update].GoName, svc.fqnForUpdate(update))
	f.Func().
		Params(g.Id("r").Op("*").Id(typeName)).
		Id(svc.methods[update].GoName).
		ParamsFunc(func(args *g.Group) {
	...
	f.Type().Id(typeName).InterfaceFunc(func(methods *g.Group) {
		...
		for _, queryOpts := range opts.GetQuery() {
			...
			if desc := handler.Comments.Leading.String(); desc != "" {
				...
				methods.Commentf("%s executes a(n) %s query", svc.methods[query].GoName, svc.fqnForQuery(query))
			}
			methods.Id(svc.methods[query].GoName).
				ParamsFunc(func(args *g.Group) {
					args.Id("ctx").Qual("context", "Context")
		...
		for _, signalOpts := range opts.GetSignal() {
			...
			if desc := handler.Comments.Leading.String(); desc != "" {
				...
				methods.Commentf("%s sends a(n) %s signal", svc.methods[signal].GoName, svc.fqnForSignal(signal))
			}
			methods.Id(svc.methods[signal].GoName).
				ParamsFunc(func(args *g.Group) {
					args.Id("ctx").Qual("context", "Context")
		...
		for _, updateOpts := range opts.GetUpdate() {
			...
			if desc := handler.Comments.Leading.String(); desc != "" {
				...
				methods.Commentf("%s executes a(n) %s update", svc.methods[update].GoName, svc.fqnForUpdate(update))
			}
			methods.Id(svc.methods[update].GoName).
				ParamsFunc(func(args *g.Group) {
					args.Id("ctx").Qual("context", "Context")
			...
			// add async flavor
			methods.Commentf("%sAsync sends a(n) %s update to the workflow", svc.methods[update].GoName, svc.fqnForUpdate(update))
