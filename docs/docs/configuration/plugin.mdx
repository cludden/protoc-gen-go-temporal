import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Plugin

Plugin options are used to globally configure the plugin's behavior at runtime.

<Tabs>
<TabItem value="buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: <flag>=<value>,<flag>=<value>
    strategy: all
```
</TabItem>
<TabItem value="protoc" label="Protoc">
```shell
--go_temporal_opt=<flag>=<value>,<flag>=<value>
```
</TabItem>
</Tabs>

## Options

### cli-categories

`bool`

Enables [cli categories](https://cli.urfave.org/v2/examples/subcommands-categories/) for grouping workflow, query, signal, and update commands.

:::info
Requires the [cli-categories](#cli-enabled) plugin option to be enabled.
:::

<Tabs>
<TabItem value="cli-categories-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: cli-enabled=true,cli-categories=true
    strategy: all
```
</TabItem>
<TabItem value="cli-categories-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=cli-enabled=true,cli-categories=true
```
</TabItem>
</Tabs>

### cli-enabled

`bool`

Enables [cli generation](/docs/guides/cli).

<Tabs>
<TabItem value="cli-enabled-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: cli-enabled=true
    strategy: all
```
</TabItem>
<TabItem value="cli-enabled-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=cli-enabled=true
```
</TabItem>
</Tabs>

### cli-v3

`bool`

Use [urfave/cli/v3](https://cli.urfave.org/v3/getting-started/) for CLI generation. Default is `false`, which uses [urfave/cli/v2](https://cli.urfave.org/v2/getting-started/).

<Tabs>
<TabItem value="cli-v3-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: 
      - cli-enabled=true
      - cli-v3=true
    strategy: all
```
</TabItem>
<TabItem value="cli-v3-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=cli-enabled=true,cli-v3=true
```
</TabItem>
</Tabs>

### disable-workflow-input-rename

`bool`

Prior to [v1.3.0](https://github.com/cludden/protoc-gen-go-temporal/releases/tag/v1.3.0), the generated workflow input struct used the `<Workflow>Input` naming convention. This caused conflicts with workflow definitions that used input messages with the same naming convetion. In `v1.3.0`, the naming convention for the generated workflow input struct was changed to `<Workflow>WorkflowInput`. In order to continue to support schemas using the old convention, this option was added to enable the old naming convention.

<Tabs>
<TabItem value="disable-workflow-input-rename-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: disable-workflow-input-rename=true
    strategy: all
```
</TabItem>
<TabItem value="disable-workflow-input-rename-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=disable-workflow-input-rename=true
```
</TabItem>
</Tabs>

### docs-out

`string`

Enables [doc generation](/docs/guides/documentation) by specifying the output path.

When the value resolves to a file path (e.g. `./proto/README.md`), all packages are rendered into a single file. When the value resolves to a directory (a path with a trailing separator, or pointing at an existing directory), one documentation file is generated per proto package, mirroring the proto source tree, with a `README.md` index at the directory root.

<Tabs>
<TabItem value="docs-out-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: docs-out=./proto/README.md
    strategy: all
```
</TabItem>
<TabItem value="docs-out-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=docs-out=./proto/README.md
```
</TabItem>
<TabItem value="docs-out-buf-dir" label="Buf (per-package)">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: docs-out=./docs/
    strategy: all
```
</TabItem>
<TabItem value="docs-out-protoc-dir" label="Protoc (per-package)">
```sh
protoc --go_temporal_opt=docs-out=./docs/
```
</TabItem>
</Tabs>

### docs-filename

`string` (default: `README.md`)

Per-package output filename in [directory mode](#docs-out). Must be a flat filename (no path separators). Ignored when `docs-out` resolves to a file.

<Tabs>
<TabItem value="docs-filename-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: docs-out=./docs/,docs-filename=README.mdx
    strategy: all
```
</TabItem>
<TabItem value="docs-filename-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=docs-out=./docs/,docs-filename=README.mdx
```
</TabItem>
</Tabs>

### docs-index

`string` (default: `README.md`)

Filename of the index file written at the root of the output directory in [directory mode](#docs-out). Must be a flat filename (no path separators). Ignored when `docs-out` resolves to a file.

<Tabs>
<TabItem value="docs-index-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: docs-out=./docs/,docs-index=index.md
    strategy: all
```
</TabItem>
<TabItem value="docs-index-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=docs-out=./docs/,docs-index=index.md
```
</TabItem>
</Tabs>

### docs-template

`string`

Specifies the path to a custom template to use for generated documentation. See the [guide](/docs/guides/documentation) for more details.

:::info
Requires the [docs-out](#docs-out) plugin option to be defined. In [directory mode](#docs-out), the template must define both `package` and `index` sub-templates.
:::

<Tabs>
<TabItem value="docs-template-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: docs-out=./proto/README.md,docs-template=./docs/custom.tpl
    strategy: all
```
</TabItem>
<TabItem value="docs-template-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=docs-out=./proto/README.md,docs-template=./docs/custom.tpl
```
</TabItem>
</Tabs>

### enable-codec

`bool`

Enables [codec server generation](/docs/guides/codec-server)

<Tabs>
<TabItem value="enable-codec-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: enable-codec=true
    strategy: all
```
</TabItem>
<TabItem value="enable-codec-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=enable-codec=true
```
</TabItem>
</Tabs>

### enable-patch-support

`bool`

Enables support for [protoc-gen-go-patch](https://github.com/alta/protopatch)

<Tabs>
<TabItem value="enable-patch-support-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: enable-patch-support=true
    strategy: all
```
</TabItem>
<TabItem value="enable-patch-support-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=enable-patch-support=true
```
</TabItem>
</Tabs>

### enable-xns

`bool`

Enables [xns generation](/docs/guides/xns)

<Tabs>
<TabItem value="enable-xns-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: enable-xns=true
    strategy: all
```
</TabItem>
<TabItem value="enable-xns-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=enable-xns=true
```
</TabItem>
</Tabs>

### ignore-acronyms

`string`

Semicolon-delimited list of acronyms to ignore during CamelCase renaming.

<Tabs>
<TabItem value="ignore-acronyms-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: 
      - ignore-acronyms=AWS;SQL
    strategy: all
```
</TabItem>
<TabItem value="ignore-acronyms-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=ignore-acronyms=AWS;SQL
```
</TabItem>
</Tabs>

### module-map [EXPERIMENTAL]

`[]string`

Repeated set of `<proto_file>=<go_import_path>` (e.g. -Mgoogle/protobuf/timestamp.proto=google.golang.org/protobuf/types/known/timestamppb)

<Tabs>
<TabItem value="module-map-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: 
      - Mgoogle/protobuf/timestamp.proto=google.golang.org/protobuf/types/known/timestamppb
    strategy: all
```
</TabItem>
<TabItem value="module-map-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=Mgoogle/protobuf/timestamp.proto=google.golang.org/protobuf/types/known/timestamppb
```
</TabItem>
</Tabs>

### nexus

`bool`

Enables experimental [Nexus generation](/docs/guides/nexus).

<Tabs>
<TabItem value="nexus-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: 
      - nexus=true
    strategy: all
```
</TabItem>
<TabItem value="nexus-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=nexus=trues
```
</TabItem>
</Tabs>

### nexus-exclude-operation-tags

`string`

Semicolon-delimited list of Nexus operation tags to exclude from generation.

<Tabs>
<TabItem value="nexus-exclude-operation-tags-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt:
      - nexus=true
      - nexus-exclude-operation-tags=disabled;internal
    strategy: all
```
</TabItem>
<TabItem value="nexus-exclude-operation-tags-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=nexus=true,nexus-exclude-operation-tags=disabled;internal
```
</TabItem>
</Tabs>

### nexus-exclude-service-tags

`string`

Semicolon-delimited list of Nexus service tags to exclude from generation.

<Tabs>
<TabItem value="nexus-exclude-service-tags-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt:
      - nexus=true
      - nexus-exclude-service-tags=disabled;internal
    strategy: all
```
</TabItem>
<TabItem value="nexus-exclude-service-tags-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=nexus=true,nexus-exclude-service-tags=disabled;internal
```
</TabItem>
</Tabs>

### nexus-include-operation-tags

`string`

Semicolon-delimited list of Nexus operation tags to include in generation.

<Tabs>
<TabItem value="nexus-include-operation-tags-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt:
      - nexus=true
      - nexus-include-operation-tags=enabled;internal
    strategy: all
```
</TabItem>
<TabItem value="nexus-include-operation-tags-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=nexus=true,nexus-include-operation-tags=enabled;internal
```
</TabItem>
</Tabs>

### nexus-include-service-tags

`string`

Semicolon-delimited list of Nexus service tags to include in generation.

<Tabs>
<TabItem value="nexus-include-service-tags-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt:
      - nexus=true
      - nexus-include-service-tags=enabled;internal
    strategy: all
```
</TabItem>
<TabItem value="nexus-include-service-tags-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=nexus=true,nexus-include-service-tags=enabled;internal
```
</TabItem>
</Tabs>

### patches

`string`

Configures how particular [Patches](/docs/guides/patches) are introduced in generated code globally.

<Tabs>
<TabItem value="cli-enabled-buf" label="Buf">
```yaml title="buf.gen.yaml"
plugins:
  - plugin: go_temporal
    out: gen
    opt: patches=64_REMOVED;65_MARKER;66_ENABLED;67_DISABLED
    strategy: all
```
</TabItem>
<TabItem value="cli-enabled-protoc" label="Protoc">
```sh
protoc --go_temporal_opt=patches=64_REMOVED;65_MARKER;66_ENABLED;67_DISABLED
```
</TabItem>
</Tabs>
