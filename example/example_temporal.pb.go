...
// NewClientWithOptions initializes a new Foo client with the given options
...
func (c *workflowClient) ExecuteLockAccount(ctx context.Context, opts *client.StartWorkflowOptions, req *LockAccountRequest) (LockAccountRun, error) {
	...
	if opts.ID == "" {
		opts.ID = req.GetAccount()
	}
	if !strings.HasPrefix(opts.ID, LockAccountIDPrefix) {
		opts.ID = fmt.Sprintf("%s/%s", LockAccountIDPrefix, opts.ID)
	}
...
func (c *workflowClient) ExecuteTransfer(ctx context.Context, opts *client.StartWorkflowOptions, req *TransferRequest) (TransferRun, error) {
	...
	if opts.ID == "" {
		opts.ID = fmt.Sprintf("%v/%v/%s", req.GetSrc(), req.GetDest(), uuid.New().String())
	}
	if !strings.HasPrefix(opts.ID, TransferIDPrefix) {
		opts.ID = fmt.Sprintf("%s/%s", TransferIDPrefix, opts.ID)
	}
	if opts.WorkflowIDReusePolicy == v1.WORKFLOW_ID_REUSE_POLICY_UNSPECIFIED {
...
func LockAccountChild(ctx workflow.Context, opts *workflow.ChildWorkflowOptions, req *LockAccountRequest) LockAccountChildRun {
	...
	if opts.WorkflowID == "" {
		opts.WorkflowID = req.GetAccount()
	}
	if !strings.HasPrefix(opts.WorkflowID, LockAccountIDPrefix) {
		opts.WorkflowID = fmt.Sprintf("%s/%s", LockAccountIDPrefix, opts.WorkflowID)
	}
	if opts.WorkflowIDReusePolicy == v1.WORKFLOW_ID_REUSE_POLICY_UNSPECIFIED {
		opts.WorkflowIDReusePolicy = v1.WORKFLOW_ID_REUSE_POLICY_ALLOW_DUPLICATE
	}
...
func TransferChild(ctx workflow.Context, opts *workflow.ChildWorkflowOptions, req *TransferRequest) TransferChildRun {
	...
	if opts.WorkflowID == "" {
		opts.WorkflowID = fmt.Sprintf("%v/%v/%s", req.GetSrc(), req.GetDest(), uuid.New().String())
	}
	if !strings.HasPrefix(opts.WorkflowID, TransferIDPrefix) {
		opts.WorkflowID = fmt.Sprintf("%s/%s", TransferIDPrefix, opts.WorkflowID)
	}
	if opts.WorkflowIDReusePolicy == v1.WORKFLOW_ID_REUSE_POLICY_UNSPECIFIED {
		opts.WorkflowIDReusePolicy = v1.WORKFLOW_ID_REUSE_POLICY_ALLOW_DUPLICATE_FAILED_ONLY
	}
